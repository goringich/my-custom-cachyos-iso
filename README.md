...
- `scripts/verify-postinstall-flow.sh` - симуляция install-time post-install path против temp target root, чтобы pre-build gate проверял не только ссылки, но и реальные target artifacts, включая user-systemd intent, portable browser-text bundle и firstboot replay с restore-audit report/self-disable.
...
- generated `system-bootstrap-firstboot.sh` после первого запуска сам снимает свой unit и кладёт `~/.local/state/system-bootstrap/restore-report.txt`
...
Пароли в ISO не встроены. Перед записью на диск установщик запрашивает два
разных пароля скрытым интерактивным вводом (минимум 12 символов). Для
автоматизированного запуска используй `--root-password-file` и
`--user-password-file`; оба файла должны иметь mode `0400` или `0600`.
Передача паролей аргументами командной строки намеренно запрещена.
...
После первой загрузки проверь вход обоих аккаунтов и удали использованные
локальные password-файлы, если применялся неинтерактивный режим.
...
- Репозитории пакетов проверяются подписями; `SigLevel = Never` запрещён CI.
